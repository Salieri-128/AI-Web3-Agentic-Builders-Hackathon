...
## 2026-06-12 - LLM Treasury Planner 与抗异常流动性
- 修改：LLM planner、转账分类记忆、稳健流动性统计、聊天方案卡和 Policy Data
- 意图：理解复合资金目标，并避免偶发大额长期抬高保留资金
- 备注：LLM 只理解和解释，公式、Pact 与执行仍由后端约束

## 2026-06-12 - 普通钱包模式与可选资金优化
- 修改：钱包快捷操作、资金优化入口、异常分类提示与 Planner 路由
- 意图：普通用户默认只处理余额、收款和转账，高级策略按需展示
- 备注：Policy Data 收入高级折叠区，Pact 边界不变

## 2026-06-12 - 修正资金优化启用顺序
- 修改：资金优化页面流程、策略状态门控和独立 Planner 对话
- 意图：先执行 Aave 策略，再解锁资金目标，避免钱包消息造成页面错位
- 备注：未启用状态不调用 Planner

## 2026-06-12 - Demo 状态重置与端到端测试路径
- 修改：本地状态重置脚本、重置测试和完整 Sepolia 验收文档
- 意图：从无历史、无本地 Pact 的状态重复验证策略、转账、收款和画像闭环
- 备注：远端 Pact 由用户在 Cobo/CAW 清理，脚本不移动链上资金

## 2026-06-12 - 收敛对话展示
- 修改：Wallet 内部滚动与自动定位、资金优化单轮消息状态
- 意图：避免连续对话撑长页面，并让 Planner 只展示当前请求结果
- 备注：Wallet 仍保留完整会话历史

## 2026-06-12 - 修复 Wallet 输入区裁切
- 修改：聊天卡片网格约束和输入区最小高度
- 意图：保证固定高度滚动时输入框和提交按钮完整可见
- 备注：消息列表继续在卡片内部滚动

## 2026-06-12 - 扩大 Wallet 聊天视区
- 修改：桌面端聊天卡片视口高度
- 意图：利用页面剩余高度展示更多消息并减少输入框下方空白
- 备注：小屏幕继续使用独立高度约束

## 2026-06-12 - 固定 Wallet 单屏布局
- 修改：Wallet 外层、工作区和聊天卡片高度约束
- 意图：禁止桌面页面整体滚动，仅允许消息列表内部滚动
- 备注：移动端保留普通页面滚动

## 2026-06-12 - 完善 Hackathon 提交材料
- 修改：README、Proposal、环境配置、测试网证据与过时文档
- 意图：提供可复现的项目说明、运行方式、安全边界和链上证据
- 备注：保留 Demo 测试手册与开发日志

## 2026-06-12 - 补充 Sepolia 交易证据
- 修改：README、Proposal
- 意图：为提交材料增加可验证的测试网交易链接
- 备注：暂不推断交易与具体操作的对应关系
